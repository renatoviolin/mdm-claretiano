jQuery(document).ready(function () {
   ...
   jQuery('#anotacao_btn_salvar').on('click', function () {
      ...
      anotacao_html = "<x-anotacao-post_it id='" + jQuery(element_mouse_selected).attr('id') + "' status=0><div class='anotacao_conteudo'>" + anotacao + "</div><div class='anotacao_excluir'>X</div></x-anotacao-post_it>";
   ...
   jQuery.ajax({
      ...
      .done(function (jsondata) {
         ...
         for (j = 0; j < items.length; j++) {
            ...
            jQuery('p[id=' + item.id_paragrafo + ']').before("<x-anotacao-post_it id='" + item.id_paragrafo + "' status=0'><div class='anotacao_conteudo'>" + item.comentario + "</div><div class='anotacao_excluir'>X</div></x-anotacao-post_it>");
         }
         jQuery("#modal").removeClass("loading");
      })
      .fail(function (jqXHR, textStatus, jsondata) {
